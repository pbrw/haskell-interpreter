...
data Program a = Program a [Decl a] [TopDef a]
  deriving (Eq, Ord, Show, Read)

instance Functor Program where
  fmap f x = case x of
    Program a decls topdefs -> Program (f a) (map (fmap f) decls) (map (fmap f) topdefs)
...
data Instr a = DInstr a (Decl a) | SInstr a (Stmt a)
  deriving (Eq, Ord, Show, Read)

instance Functor Instr where
  fmap f x = case x of
    DInstr a decl -> DInstr (f a) (fmap f decl)
    SInstr a stmt -> SInstr (f a) (fmap f stmt)

data Decl a = CostBlk a Ident (Block a) | Var a (Type a) [Item a]
  deriving (Eq, Ord, Show, Read)

instance Functor Decl where
  fmap f x = case x of
    CostBlk a ident block -> CostBlk (f a) ident (fmap f block)
    Var a type_ items -> Var (f a) (fmap f type_) (map (fmap f) items)

data Block a = Block a [Instr a]
  deriving (Eq, Ord, Show, Read)

instance Functor Block where
  fmap f x = case x of
    Block a instrs -> Block (f a) (map (fmap f) instrs)

data LoopBlk a = LoopBlk a [Instr a]
  deriving (Eq, Ord, Show, Read)

instance Functor LoopBlk where
  fmap f x = case x of
    LoopBlk a instrs -> LoopBlk (f a) (map (fmap f) instrs)
...
instance Functor Stmt where
  fmap f x = case x of
    ...
    Break a -> Break (f a)
    MultiBrk a expr -> MultiBrk (f a) (fmap f expr)
